...
- 🏆 **[v6 ReClor downstream](extensions/reports/V6_RECLOR.md)** — DeBERTa-large fine-tuned on ReClor on top of v5 vs v6 backbones, same hparams. **v6 wins +0.8 pp on dev_acc (63.6% vs 62.8%)** and leads at every evaluation step.
- 📉 **[v6 LogiQA downstream](extensions/reports/V6_LOGIQA.md)** — same setup on LogiQA: **v6 *loses* −1.8 pp (39.17% vs 41.01%)**. Honest counterexample: cleaner T5 helps ReClor but hurts LogiQA, likely because v6 dropped the legacy double_negation rows (1.3% of corpus) that LogiQA's deductive questions depend on.
- 🌐 **[Held-out PARARULE generalization](extensions/reports/HELDOUT_PARARULE.md)** — v4 T5 on 60 unseen Depth5 sentences: pass rate **70.6% → 72.0%** (+1.4 pp). Smaller than in-distribution but consistent direction; double_negation generalizes best (+6 items), implication is brittle (−3).
- 🔧 **[De Morgan-aware contraposition fix](extensions/reports/RULEFIX_DEMORGAN.md)** — root-cause fix for the 3 contraposition cases the v4 T5 fine-tune couldn't recover: rule wasn't distributing negation over conjunctive antecedents. Full pilot pass rate now **68.9% → 82.2%** (+13.3 pp end-to-end), contraposition **15/15 perfect** on the pilot; held-out +1.4 pp on top of v4.
